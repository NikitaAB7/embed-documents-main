...
# Tracing support (lazy import to avoid circular deps)
_tracer = None
    ...
    async def retrieve(
        ...
        if tracer and tracer.is_enabled:
            span = tracer.trace_span(
                ...
                run_type="retriever",
                inputs={"query": query, "k": k, "use_dynamic_filters": use_dynamic_filters},
        ...
        try:
            ...
            if not route.use_rag:
                result = PipelineResult(route=route, documents=[])
                if span:
                    span.set_outputs({"num_docs": 0, "route": "no_rag"})
                return result
            ...
            if span:
                span.set_outputs({
                    "num_docs": len(reranked.documents),
